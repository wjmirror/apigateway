...
namespace ApiGateway.CRM
{
    public class CRMService
    {
        ...
        private static ConcurrentDictionary<string, string> s_userObjectIdCache = new ConcurrentDictionary<string, string>();

        public async Task<string> GetUserObjectIdAsync(IIdentity identity)
        {
            string userNetworkId = identity.Name;
            if (s_userObjectIdCache.ContainsKey(userNetworkId))
            {
                return s_userObjectIdCache[userNetworkId];
            }
            ...
            string authToken = await this.GetOAuthTokenAsync();
            httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {authToken}");

            string query = $"systemusers?$filter=ssco_networkid eq '{userNetworkId}' and deletedstate eq 0 &$select=ssco_networkid,azureactivedirectoryobjectid,systemuserid";
            ...
            if (ret.ContainsKey("value"))
            {
                ...
                if (users != null && users.Count == 1)
                {
                    ...
                    s_userObjectIdCache[userNetworkId] = objectId;
                }
            }
